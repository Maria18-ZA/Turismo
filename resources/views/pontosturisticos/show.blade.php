...
@section('content')
    ...
    <table border="1">
        ...
        <tr>
            <th>Imagem</th>
            <td>
                @if($pontoTuristico->imagens && $pontoTuristico->imagens->isNotEmpty())
        ...
        <tr>
            <th>Criado em</th>
            <td>
                {{ optional($pontoTuristico->created_at)?->format('d/m/Y H:i') ?? 'Não disponível' }}
            </td>
        </tr>

    </table>

    <a href="{{ route('pontosturisticos.edit', $pontoTuristico) }}">
        Editar
    </a>
